...
var MumbleConnection = function( socket, options ) {
    ...
    this.authSent = false;
    this.ignoreNormalTalking = false;
    this.ignoreLastAudioFrame = false; // see https://github.com/mumble-voip/mumble/issues/2604
 ...
 **/
var encodeVersion = function( major, minor, patch ) {
 ...
 * @param {string[]} tokens - Access tokens
 **/
 ...
 *
 * @param {string} type Message type ID
...
MumbleConnection.prototype.joinPath = function( path ) {
    ...
    for( var i in path ) {
        ...
        if( nextChannel === null ) { util.warn( 'Path not found!' ); break; }
        channel = nextChannel;
    }

    // Send a new user state to update the current channel.
    this.sendMessage( 'UserState', {
    ...
    while( this.voiceBufferLength >= this.packetBuffer.length ) {
        ...
        while( written < this.packetBuffer.length ) {
            ...
            if( written > this.packetBuffer.length ) {
                ...
                this.voiceBuffer[ 0 ] = this.voiceBuffer[ 0 ].slice( writtenFromThis );
                ...
                this.voiceBufferLength =
                    this.voiceBufferLength - this.voiceBuffer[ 0 ].length;
 ...
 **/
MumbleConnection.prototype.sendVoiceFrame = function(
        frame, whisperTarget, voiceSequence ) {

    if( this.initPending ) {
        throw new Error(
            'Wait for "initialized" event before attempting to send audio.' );
    }
    ...
    var frames = this.sendEncodedFrame(
            encoded, this.codec, whisperTarget, voiceSequence );
 ...
 **/
MumbleConnection.prototype.sendEncodedFrame = function(
        packets, codec, whisperTarget, voiceSequence ) {
...
MumbleConnection.prototype._processMessage = function( type, msg ) {
    ...
    if( !this[ '_on' + Messages.nameById[ type ] ] ) {
        util.trace( 'Unhandled message:' + Messages.nameById[ type ] );
        util.trace( Messages.nameById[ type ] );
        util.trace( msg );
    } else {
    ...
    if( this.initPending ) {
        ...
        if( this.initPending.length === 0 ) {
            this.initialize();
            util.trace( 'Mumble connection initialized.' );
 ...
 * @private
 *
 * @param {Object} codecVersion Codec message
 **/
 ...
 * @private
 *
 * @param {Object} data Details about the error
 **/
 ...
 * @private
 *
 * @param {Object} channelData Channel state message
 **/
 ...
 * @private
 *
 * @param {Object} userState User state message
 */
 ...
 * @private
 *
 * @param {Object} syncData Server sync message
 **/
 ...
 * @private
 *
 * @param {Object} reject Reject message
 **/
...
MumbleConnection.prototype._onUDPTunnel = function( data ) {
    ...
    // Ignore the packet if we are told to ignore normal talking and the packet has target = 0 (normal talking)
    if( target === 0 && this.ignoreNormalTalking)
        return;
...
MumbleConnection.prototype._processAudio = function() {
    ...
    while( this.lastProcess + this.FRAME_LENGTH < Date.now() ) {
        ...
        for( var p in packets ) {
            ...
            if( packet.frame && !user.talking ) {
                ...
                this.emit( 'voice-start', {
                    session: user.session, name: user.name, talking: true } );
            }
            ...
            if( packet.terminator ) {
                ...
                this.emit( 'voice-end', {
                    session: user.session, name: user.name, talking: false } );
            }
        }
        ...
        for( var f in packets ) {
            ...
            // ignore the last bugged frame if we listen only to whispers/shout
            // see https://github.com/mumble-voip/mumble/issues/2604
            if( packet.terminator &&
                this.ignoreLastAudioFrame &&
                this.ignoreNormalTalking ) {
                    continue;
            }
 ...
 * @private
 *
 * @returns {Object} Packets dequeued.
 */
 ...
 *
 * @returns {Buffer} Mixed audio sample buffer
 */
 ...
 * @private
 *
 * @param {number} parentId Parent channel ID
 * @param {string} name Channel name to be looked up
 * @param {bool} caseInsensitive true to perform case insensitive name comparison
 *
...
MumbleConnection.prototype._getDecoder = function( user, codec ) {
    if( codec === MumbleConnection.codec.Opus ) {
        ...
        return ( user.celtDecoder = new CeltEncoder(
                this.SAMPLING_RATE, this.FRAME_SIZE ) );
    }
};
